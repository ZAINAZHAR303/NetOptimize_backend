...
async def analyze_policy(request: PolicyAnalysisRequest):
    try:
        ...
        prompt = f"""
        You are an AI expert in telecom policies. Provide a concise summary, risk assessment, and recommendations.
...
async def optimize_network(request: NetworkDesignRequest):
    # Include system instructions in the prompt
    prompt = f"""
    You are an AI specializing in network optimization. Provide a structured network plan, cost estimate, and coverage percentage.
...
async def resource_allocation(request: ResourceOptimizationRequest):
    # Include system instructions in the prompt
    prompt = f"""
    You are an AI expert in telecom resource management. Provide underutilized assets, suggested allocation, and cost savings.
